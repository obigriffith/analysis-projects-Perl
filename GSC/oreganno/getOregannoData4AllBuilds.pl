...
unless($opt_t && $opt_o){
  ...
  print "NOTE: 'REGULATORY HAPLOTYPE' records and Deprecated records are currently excluded\n";
  print "NOTE: for UCSC option, only records that are 'MAPPED' and have 'POSITIVE OUTCOME' are included.\n";
  print "NOTE: for FULL option, all non-haplotype and non-deprecated records (even those NOT MAPPED and with outcomes other than POSITIVE) are included\n";
  print "NOTE: FULL option also includes more details than UCSC such as actual sequences.  However, comments and many evidence details are still missing\n";
  exit;
}
...
print OUTFILE "Species\tUCSC Build\tBuild\tMapping status\tOutcome\tchrom\tchromStart\tchromEnd\tStrand\tname\tlandMarkId\tlandMarkType\tGene name\tGene ID\tGene Source\tTF name\tTF ID\tTF Source\tdbSNP ID\tPMID\tDataset\tEvidence Subtypes\n";
}
...
while (my $row_ref = $sth_rec -> fetchrow_arrayref()) {
	...
	print "Retrieving details for $stable_id\n";
	...
	#Get mapping information for all mappings
        my $SQL_GET_MAPPING = "SELECT mapping.*, ucsc_build_name, build_name FROM mapping, mapping_genome WHERE mapping.record_id=\"$record_id\" AND mapping.mapping_genome_id=mapping_genome.id ORDER BY mapping_genome_id DESC";
	...
	#For each mapping print out details.  Only mapping info will change.
	while (my $row_ref = $sth_map -> fetchrow_arrayref()) {
	  ...
	  if ($file_type eq "UCSC") {
	    if ($mapping_status eq "MAPPED" && $outcome eq "POSITIVE OUTCOME") {
	      print OUTFILE "$species\t$ucsc_build\t$build\t$mapping_status\t$outcome\t$sequence_region_name\t$start\t$end\t$strand\t$stable_id\t$stable_id\t$type\t$gene_name\t$gene_id\t$gene_source\t$tf_name\t$tf_id\t$tf_source\t$db_snp_id\t$pubmed_id\t$dataset\t$evidence_subtypes\n";
	    }
	  }
