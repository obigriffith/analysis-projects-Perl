...
getopts("f:o:n:rl");
use vars qw($opt_f $opt_o $opt_l $opt_n $opt_r);
...
if ($opt_o){
  ...
  foreach my $tag (sort keys %Data){
	...
	if ($Data{$tag}{$exp}){
	  $data = $Data{$tag}{$exp};
	  if ($opt_r){
	    my $freq_data = ((($data + 1)*10000)/($Exp{$series}{$exp}));
	    $data = $freq_data;
	  }
	  if ($opt_l){
...
print "-r option to specify frequencies instead of raw data (((tagcount + 1) x 10000)/total tag count)\n";
