...
foreach my $gene (sort{$a<=>$b} keys %GENES){
  ...
  my $max_fold_change = 0;
  my $min_fold_change = 0;
  my @PMIDS;
  my @FCS; #Array for storing fold changes for each gene
  foreach my $study (keys %{$GENES{$gene}}){
	...
	push (@FCS, $GENES{$gene}{$study}{$comp}{'Std_Fold_Change'});
      }
  ...
  #Determine average fold change (there are several ways to do this)
  $avg_fold_change_nonabs=$net_fold_change/$comp_count;
  $avg_fold_change=abs($net_fold_change/$comp_count);
  if ($fold_change_comp_count>0){ #If no studies reported a fold-change value, leave the average values as zero
  ...
  #Determine max and min fold change for range
  if(@FCS){
    my @FCS_sorted = sort{$b<=>$a} @FCS;
    my @FCS_rev_sorted = sort{$a<=>$b} @FCS;
    $max_fold_change=$FCS_sorted[0];
    $min_fold_change=$FCS_rev_sorted[0];
  }
  print "$gene\t$GENELIST{$gene}{'Entrez_name'}\t$study_count\t$comp_count\t$total_sample_size\t$avg_fold_change_nonabs_real\n";
  ...
  $GENESTATS{$gene}{'up_down'}="$up_count/$down_count";
  $GENESTATS{$gene}{'avg_fold_change_nonabs_real'}=$avg_fold_change_nonabs_real;
  $GENESTATS{$gene}{'fold_change_comp_count'}=$fold_change_comp_count;
  $GENESTATS{$gene}{'fold_change_total_sample_size'}=$fold_change_total_sample_size;
  $GENESTATS{$gene}{'max_fold_change'}=$max_fold_change;
  $GENESTATS{$gene}{'min_fold_change'}=$min_fold_change;
  ...
  $GENESUMMARY{$disagreement_count}{$comp_count}{$total_sample_size}{$avg_fold_change_real}{$gene}{'max_fold_change'}=$max_fold_change;
  $GENESUMMARY{$disagreement_count}{$comp_count}{$total_sample_size}{$avg_fold_change_real}{$gene}{'min_fold_change'}=$min_fold_change;
}

#Finally, Add gene summary stats back to %DATA hash
foreach my $study (sort{$a<=>$b} keys %DATA){
  foreach my $entry (sort{$a<=>$b} keys %{$DATA{$study}}){
    my ($n_studies, $n_comparisons, $n_total_samples, $net_fold_change, $net_direction, $net_direction_score, $n_disagreements, $up_down, $avg_fold_change_nonabs_real, $fold_change_comp_count, $fold_change_total_sample_size, $max_fold_change, $min_fold_change);
    my $entrez_id = $DATA{$study}{$entry}{'Entrez_id'};
    if ($entrez_id eq 'NA'){#Set gene stats to NA by default in case no gene entry
      $n_studies = 'NA'; $n_comparisons = 'NA'; $n_total_samples = 'NA'; $net_fold_change='NA'; $net_direction='NA'; $net_direction_score='NA',$n_disagreements='NA'; $up_down='NA'; $avg_fold_change_nonabs_real='NA'; $fold_change_comp_count='NA'; $fold_change_total_sample_size='NA'; $max_fold_change='NA'; $min_fold_change='NA';
    }else{
      ...
      $up_down=$GENESTATS{$entrez_id}{'up_down'};
      $avg_fold_change_nonabs_real=$GENESTATS{$entrez_id}{'avg_fold_change_nonabs_real'};
      $fold_change_comp_count=$GENESTATS{$entrez_id}{'fold_change_comp_count'};
      $fold_change_total_sample_size=$GENESTATS{$entrez_id}{'fold_change_total_sample_size'};
      $max_fold_change=$GENESTATS{$entrez_id}{'max_fold_change'};
      $min_fold_change=$GENESTATS{$entrez_id}{'min_fold_change'};
    }
    ...
    $DATA{$study}{$entry}{'up_down'}=$up_down;
    $DATA{$study}{$entry}{'avg_fold_change'}=$avg_fold_change_nonabs_real;
    $DATA{$study}{$entry}{'n_comparisons_FC'}=$fold_change_comp_count;
    $DATA{$study}{$entry}{'n_total_samples_FC'}=$fold_change_total_sample_size;
    $DATA{$study}{$entry}{'max_fold_change'}=$max_fold_change;
    $DATA{$study}{$entry}{'min_fold_change'}=$min_fold_change;
  }
}
...
splice(@headers, 2, 0, ('Entry','Entrez_id','Entrez_name','Entrez_description','Map_method','n_studies','n_comparisons','n_total_samples','net_fold_change','net_direction','net_direction_score','n_disagreements','up_down','avg_fold_change','n_comparisons_FC','n_total_samples_FC','max_fold_change','min_fold_change'));
...
if ($opt_t){
  ...
  print TABLEFILE "Entrez_Name\tEntrez_ID\tEntrez_Description\tN_Comparisons\tUp/Down(N_Comparisons_Fold_Change)\tTotal_Samples(Total_Samples_Fold_Change)\tAvg_Fold_Change\tPMIDS\n";
  foreach my $n_disagreements (sort{$a<=>$b} keys %GENESUMMARY){
	...
	foreach my $avg_fold_change (sort{$b<=>$a} keys %{$GENESUMMARY{$n_disagreements}{$n_comparisons}{$total_samples}}){
	  foreach my $gene (sort{$a<=>$b} keys %{$GENESUMMARY{$n_disagreements}{$n_comparisons}{$total_samples}{$avg_fold_change}}){
	    ...
	    my $fold_change_comp_count = $GENESUMMARY{$n_disagreements}{$n_comparisons}{$total_samples}{$avg_fold_change}{$gene}{'fold_change_comp_count'};
	    my $fold_change_total_sample_size = $GENESUMMARY{$n_disagreements}{$n_comparisons}{$total_samples}{$avg_fold_change}{$gene}{'fold_change_total_sample_size'};
	    my $max_fold_change = $GENESUMMARY{$n_disagreements}{$n_comparisons}{$total_samples}{$avg_fold_change}{$gene}{'max_fold_change'};
	    my $min_fold_change = $GENESUMMARY{$n_disagreements}{$n_comparisons}{$total_samples}{$avg_fold_change}{$gene}{'min_fold_change'};
	    print TABLEFILE "$entrez_name\t$entrez_id\t$entrez_description\t$n_comparisons\t$up_down ($fold_change_comp_count)\t$total_samples ($fold_change_total_sample_size)\t$avg_fold_change_nonabs_real ($min_fold_change to $max_fold_change)\t$PMIDS\n";
	    print "$entrez_name\t$entrez_id\t$entrez_description\t$n_comparisons\t$up_down ($fold_change_comp_count)\t$total_samples ($fold_change_total_sample_size)\t$avg_fold_change_nonabs_real ($min_fold_change to $max_fold_change)\t$PMIDS\n";
	  }
	}
      }
    }
