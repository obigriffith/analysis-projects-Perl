...
while (<ANNOTATIONS>){
  ...
  my @entry=split("\t", $_);
  my $sample=$entry[0];
  my $source=lc($entry[1]); #Change everything to lower case to prevent identical terms with differences in case from being considered separately
  my $primary_site=lc($entry[2]); #This is highly redundant with 'source' and might not be worth including
  my $histology=lc($entry[3]);
  my $gender=lc($entry[4]);
  my $ethnicity=lc($entry[5]);
  my $tobacco_use=lc($entry[6]);
  my $alcohol_use=lc($entry[7]);

  #Need to distinguis NA's and other ambguous terms in different term categories
  $source="source_"."$source";
  $histology="histology_"."$histology";
  $gender="gender_"."$gender";
  $ethnicity="ethnicity_"."$ethnicity";
  $tobacco_use="tobacco_"."$tobacco_use"; #Make tobacco terms more descriptive than yes/no/na
  if ($tobacco_use=~/\d+\-\d+/){$tobacco_use="tobacco_yes";}#For a few samples a range of years is provided.  These are too few to be useful.  Convert to "yes"
  $alcohol_use="alcohol_"."$alcohol_use";

  #Create a hash of all terms.
  $terms{$source}{$sample}++;
  $terms{$histology}{$sample}++;
  $terms{$gender}{$sample}++;
  $terms{$ethnicity}{$sample}++;
  $terms{$tobacco_use}{$sample}++;
  $terms{$alcohol_use}{$sample}++;
}
close ANNOTATIONS;
