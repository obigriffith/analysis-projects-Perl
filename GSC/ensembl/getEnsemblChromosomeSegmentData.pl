...
print GREEN, "\n\nLocal example: getEnsemblChromosomeSegmentData.pl  --ensembl_api_version=35  --connect_type=local  --ensembl_server=ensembl01.bcgsc.ca  --ensembl_user=ensembl  --ensembl_password=ensembl  --species=Human  --segment_file=ExampleSegments_hg17.txt\n", RESET;
...
print GREEN, "\n\tThe species using: --species (e.g. --species=Human or --species='Homo sapiens')", RESET;
print GREEN, "\n\t\tThe 'Core' database for this species for the EnsEMBL version you specified above will be used automatically", RESET;
print GREEN, "\n\nWeb example: getEnsemblChromosomeSegmentData.pl  --ensembl_api_version=35  --connect_type=remote  --species=Human  --segment_file=ExampleSegments_hg17.txt\n", RESET;
...
print GREEN, "\n\nWeb example: getEnsemblChromosomeSegmentData.pl  --ensembl_api_version=31  --connect_type=legacy  --ensembl_database=homo_sapiens_core_31_35d  --ensembl_server=ensembldb.ensembl.org  --ensembl_user=anonymous  --ensembl_password=anything  --segment_file=ExampleSegments_hg17.txt\n\n", RESET;


unless (($ensembl_api_version =~ /^\d+/) && $segment_file && $connect_type){
...
open (OUT, ">chromosome_segs.fa") || die "\nCould not open output file\n\n";
foreach my $seg_number (sort {$a <=> $b} keys %segs){
  ...
  print BLUE, "\nGetting sequence and genes for region: $chr:$start-$end\n";
  ...
  foreach my $gene (@genes){
    ...
    print YELLOW, "\n\tGene: $stable_id\t$gene_info{$stable_id}{chromosome} ($gene_info{$stable_id}{strand}):$gene_info{$stable_id}{start} - $gene_info{$stable_id}{end})", RESET;
  }
  print "\n";
}
